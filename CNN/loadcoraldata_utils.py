...
#### Generate a training set (stored in class)
# Input:
# 	image_size: Size of image
# 	N_train: Number of images per class
# 	idxremove: Which index to remove (NOTE: This is NOT the channel index, but is from the index of a list of datasets)
# 	figureson: True/False, displays 1 image from each class
...
#### Generate a validation set (stored in class)
# Same inputs as function generate_trainingset
...
#### Generate a test set (stored in class)
# Same inputs as function generate_trainingset
...
#### Export a number of patches and segmented maps to a target directory
# Input:
...
# 	labelkey: Naming convention of class labels (NOTE: must be same # as the # of classes)
	def export_segmentation_map(self, exporttrainpath, exportlabelpath, image_size=25, N=20000, lastchannelremove = True, labelkey = None):
		...
		if len(labelkey) != self.num_classes:
